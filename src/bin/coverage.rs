...
impl Config {
    fn load() -> Result<Config, failure::Error> {
        let config = match fs::read_to_string("snmp-grammar.toml") {
...
type ModuleContentsHandle =
    parse::OwnedHandle<proc_macro2::TokenStream, parse::Stmt<'static, 'static, proc_macro2::TokenStream>>;
...
fn print_statistics(counters: Counters) {
    ...
    println!("* {} parsed fully and unambiguously", counters.unambiguous_count);
    println!("* {} parsed fully (but ambiguously)", counters.ambiguous_count);
    println!("* {} parsed partially (only a prefix)", counters.too_short_count);
    println!("* {} didn't parse at all (lexer error?)", counters.no_parse_count);
}

fn main() -> Result<(), failure::Error> {
    match Command::from_args() {
        Command::File { graphviz_forest, file } => {
